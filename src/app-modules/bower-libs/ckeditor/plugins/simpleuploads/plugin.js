...
(function () {
	...
	function PickAndSendFile(editor, forImage, caller, callback) {
		if (IEUpload_fileName) {
			showMessage(editor, 'Please, wait to finish the current upload');
			return;
		}
		...
		if (typeof FormData == 'undefined') {
					...
					window.setTimeout(function () {
						...
						picker.onchange = function () {
							...
							if (evdata.requiresImage && !CKEDITOR.plugins.simpleuploads.isImageExtension(filePickerEditor, evdata.name)) {
								showMessage(editor, filePickerEditor.lang.simpleuploads.nonImageExtension);
								return;
							}
			...
			iframe.onreadystatechange = function () {
				if (iframe.readyState == 'complete') {
					window.setTimeout(function () {
						if (IEUpload_fileName) {
							showMessage(editor, 'The file upload has failed');
	...
	function setUrl(fileUrl, msg) {
		...
		if (fileUrl) {
			...
			if (filePickerForceLink) {
				...
				element.on('error', errorListener, null, { editor: editor, element: element });
			}
	...
	*/
	function receivedUrl(fileUrl, data, editor, el, attribute) {
		if (el.$.nodeName.toLowerCase() == 'span') {
			...
			if (data.originalNode) {
				...
				img.removeAttribute('margin-top');
				img.removeAttribute('margin-bottom');
				img.removeAttribute('margin-left');
				img.removeAttribute('margin-right');
				img.style.width = '';
				img.style.height = '';
				img.style.marginTop = "";
		...
		init: function (editor) {
			...
			editor.on('paste', function (e) {
				...
				if (CKEDITOR.env.webkit && html.indexOf('webkit-fake-url') > 0) {
					showMessage(editor, 'Sorry, the images pasted with Safari aren\'t usable');
	...
	function errorListener(e) {
		...
		e.removeListener();
		showMessage(editor, 'Failed to load the image with the provided URL: "' + e.sender.data('cke-saved-src') + '"');
		element.remove();
	}
		...
		xhr.onload = function () {
			...
			if (xhr.status != 200) {
				if (xhr.status == 413)
					...
					showMessage(editor, 'Error posting the file to ' + data.url + '\r\nResponse status: ' + xhr.status);
	...
	function createSimpleUpload(editor, dialogName, definition, element) {
		...
		var filebrowser = element.filebrowser,
			...
			targetField = filebrowser.target.split(':');
