	...
	dataTable: function ($adapter, $scope) {
		...
		const dataTableCfg = {
			...
			getCurrenciesDatasource() {
				$scope.dataSet.currencies = {
					ownerId: $scope.dataSet.currentOwner,
					items: []
				};
				let currencies = $scope.dataSet.currentOwner.currencies || "";
				currencies.split(",").forEach(currency => {
					$scope.dataSet.currencies.items.push({id: currency});
				});
				console.log($scope.dataSet.currencies);
			},
			createApiArgs: function () {
				this.getCurrenciesDatasource();
				return {
					ownerId: $scope.dataSet.currentOwner.id,
				};
			},
			createCallback: function (dataItem) {
				...
				dataItem.currency = $scope.dataSet.currencies.items.length ? $scope.dataSet.currencies.items[0].id : "EUR"
				return dataItem;
			},
			exportCfg: {
				item: {
					getPreviousMonthName(source) {
						...
						date.setDate(1);
						let locale = (applicationInfo.language ==="ro")? "ro-RO":"en-GB";
						console.log("locale", locale, applicationInfo);
						const previousMonth = date.toLocaleString(locale, {month: "long"});
			...
			details: {
				dataAdapter: $adapter({
					...
					extend: function (dataItem) {
						...
						this.vatValue = ((this.vat * this.value)/100).round(2);
					}
				}),
				createCallback: function (dataItem) {
					dataItem.quantity = 0;
				...
				validateField(fieldName, dataItem) {
					...
					switch (fieldName) {
						case "unit":
							 ...
							 */
							let ctrl = this.$ctrl.parentConfig.$ctrl;
							let currency = this.$ctrl.parentConfig.$ctrl.$$grid.$$editor.form.$ctrl.datasource.currency;
							if (value && !dataItem.price && currency === "RON") {
						...
						case "price":
							...
							dataItem.vatValue = (dataItem.vat * dataItem.value)/100;
							break;
					}
					return true;
				},
		...
		dataTableCfg.getCurrenciesDatasource();
		return dataTableCfg;
	}
};
