(function (window, angular) {
	...
	function template($element, $attrs) {
		...
		if (!$attrs.itemIdField) console.error("For autocomplete must provide a item-id-field");
		if (!$attrs.itemDisplayField) $attrs.itemDisplayField = $attrs.itemIdField;
		...
		let dropdown = createElement("ax-dropdown-list", {
			...
			listItemDisplayField: $attrs.itemDisplayField,
			listWidth: "100%",
		...
		if ($attrs.linkPopupTemplateUrl) {
			...
			let disabled = "";
			if ($attrs.ngReadonly) disabled = "$ctrl.ngReadonly()";
			if ($attrs.ngDisabled) disabled = (disabled ? disabled + " && " : "") + "$ctrl.ngDisabled";
			popup.setAttribute("ng-disabled", disabled);
	...
	// datasource for popup is $ctrl.filteredDatasource
	...
	function controller(scope, element, attrs, $timeout) {
		...
		let blurHandle = function ($event) {
			// debug && console.log("ngBlur ?", $event, self.ngModel, "stopFireBlur", stopFireBlur, scope.$parent.$parent.$parent.dataItem);
			...
			$timeout(function () {
				if (self.modelChanged && attrs.saveData) {
					debug && console.log("saveData executed", self.ngModel, attrs.saveData, scope.$parent.$parent.$parent.dataItem);
		...
		var modelChanged = function (newValue, oldValue) {
			self.modelChanged = newValue !== oldValue || self.displayValueChanged;
			if (!self.modelChanged && !self.loading) return;
			debug && console.log("model changed:", self.modelChanged, 'newValue:', newValue, 'oldValue', oldValue);
			let dataSource = getDatasource();
			// console.log("datasource", dataSource, self.selectedItem?self.selectedItem[attrs.itemIdField]:undefined);
			if ((!self.selectedItem || self.selectedItem[attrs.itemIdField] !== newValue) && newValue !== undefined) {
				...
				debug && console.log("find selectedItem", self.selectedItem);
			}
			if (newValue === undefined) self.selectedItem = undefined;
			if (self.selectedItem) self.filteredDatasource = dataSource;
			debug && console.log("selectedItem ", self.selectedItem, 'filteredDatasource', self.filteredDatasource);
		...
		scope.$watch("$ctrl.datasource", function () {
			...
			else modelChanged(self.ngModel, undefined);
		});
		const noIdField = attrs.itemDisplayField === attrs.itemIdField;
		if (noIdField) scope.$ctrl.displayValue = scope.$ctrl.ngModel;
		scope.$watch("$ctrl.displayValue", function (value) {
			if (element.hasAttribute("readonly") || element.hasAttribute("disabled")) return;
			debug && console.log("displayValue", self.displayValue, 'selectedDisplayValue', self.selectedDisplayValue, 'newValue:', value);
			...
			if (value === "") {
				...
				const valueToLowerCase = attrs.itemInvariantField ? self.invariant(value).toLowerCase() : value.cleaning().toLowerCase();
				self.filteredDatasource = getDatasource().filter(function (item, i, data) {
					...
					return (item[self.invariantField].toLowerCase().includes(valueToLowerCase));
				}, this);
				debug && console.log("filteredDatasource", self.filteredDatasource);
				if (noIdField) {
					self.ngModel = value;
					if (self.popupCtrl.table && self.popupCtrl.table.$ctrl) self.popupCtrl.table.$ctrl.datasourceSet(self.filteredDatasource);
				} else if (self.filteredDatasource.length === -1) {
					...
					self.ngModel = noIdField ? value : undefined;
