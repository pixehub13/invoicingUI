class axTableTemplate extends axElement {
	...
	computeDefs() {
		...
		this.forEachColumn(function (item) {
			...
			if (hideableColumn.exportable) {
				hideableColumn.exportColumnIndex = exportColumnIndex;
				item.setAttribute('export-column-index', exportColumnIndex)
			...
			if (template.attributes["no-header"] !== "true") {
				...
				var addHeader = function (headerDef) {
					...
					if (!sortableAdded && headerDef.hasAttribute("sortable") && (parseInt(headerDef.getAttribute("colspan")) > 1) === false) {
						controller.columns.sortable.push({field: headerDef.getAttribute("sortable"), title: trans(header,true), index: columnIndex});
