	...
	createSettingsContainer(attributes, dataTableTemplate) {
		...
		if (dataTableTemplate.attributes["customizable-edit-mode"] !== "false") {
			let cfgEditMode = createElement("ax-dropdown-popup",
				{
					tabindex: -1,
					"btn-class": "btn icon fa fa-pencil",
					"caret-class": "fa ",
					"close-on-mouseleave1": "true",
					"close-on-escape": "true",
					"popup-class": "dropdown-popup-menu",
		...
		// console.log('edit-row', dataTableTemplate.attributes["edit-row"], dataTableTemplate.attributes);
		if (dataTableTemplate.attributes["edit-row"] === "editor") {
			...
			if (dataTableTemplate.attributes["show-commands-tooltips"]) editBtn.setAttribute("uib-tooltip", dataTableTemplate.getMessage('toolbar', 'showEditor'));
