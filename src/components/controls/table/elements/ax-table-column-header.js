class axTableColumnHeader extends axElement {
	...
	create(element, dataTableTemplate) {
		...
		if (this.source.hasAttribute("sortable") && !hasColspan) {
			...
			var wrapper = createElement('div', {"style": "position:relative;", class: "inline"}, trans(this.source.innerHTML,true) + span.outerHTML);
			this.source.innerHTML = wrapper.outerHTML;
		}
		if (hasColspan || this.source.getAttribute("header-menu") === "false" || th.hasClass("empty-column")) {
			th.innerHTML = trans(this.source.innerHTML.trim(),true)  || "<div style='color:transparent;overflow: hidden;height:0;'>Empty</div>";
			...
			let buttonContent = trans(this.source.innerHTML,true);
