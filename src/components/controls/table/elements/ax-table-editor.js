class axTableEditor {
				...
				form: {
					...
					export: {
						...
						sendAsEmailChanged: function () {
							if (!this.outputOptions.includes(this.excelTab)) this.outputOptions.push(this.excelTab);
							if (editor.form.enabledExportPdf() && !this.outputOptions.includes(this.pdfTab)) this.outputOptions.push(this.pdfTab);
							if (editor.form.enabledExportEmail()) this.hasEmail = true;
							else this.sendAsEmail = false;
							if (this.sendAsEmail) {
								...
								if (this.outputOptions.includes(this.excelTab)) {
									let index = this.outputOptions.indexOf(this.excelTab);
									this.outputOptions.splice(index, 1);
									if (this.output === this.excelTab) this.output = this.outputOptions[0];
								}
							} else {
								if (!this.outputOptions.includes(this.browserTab)) {
								...
								if (!this.outputOptions.includes(this.excelTab)) {
									this.outputOptions.push(this.excelTab);
								}
					...
					onInitDone: function () {
						...
						this.enabledExport = $controller.config.editor.enabledExport || function () {
							return true;
						};
						this.enabledExportEmail = $controller.config.editor.enabledExportEmail || function () {
							return false;
						};
						this.enabledExportPdf = $controller.config.editor.enabledExportPdf || function () {
							return false;
						};
						...
						// nu e bine: aduce toate metodele din axFromCtrl in config (validateField, validateForm,...)
	...
	getTranslation(source) {
		...
		let translated = defContent.startsWith("trans:") ? trans(defContent.substring(6), false) : defContent;
		return translated;
	}
	...
	static createPopupForm(template) {
		...
		let editorTitle = def.attr("editor-title") || "'Current DataItem'";
		if (editorTitle.startsWith('trans:')) {
			let editorTranslated = trans(editorTitle,true);
			createElement("div", {}, editorTranslated, formTitle);
		} else createElement("div", {ngBind: def.attr("editor-title") || "'Current DataItem'"}, "", formTitle);
		axTableEditor.createEditorBtns(formTitle, template);
		createElement("div", {
	...
	static createEditorBtns(toolbar, template) {
		let container = createElement('div', {role: "editor-btn-container"})
		createElement("span", {
		...
		}, "<i class='fa fa-edit'></i>", container);
		createElement("span", {
			...
			class: "btn btn-primary btn-edit-popup"
		}, "<i class='fa fa-plus'></i>", container);
		createElement("span", {
			ngDisabled: "grid.$$editor.form.readOnly",
			ngShow: "grid.$$editor.form.canEdit()",
			...
			class: "btn btn-primary btn-edit-popup"
		}, "<i class='fa fa-save'></i>", container);
		createElement("span", {
			ngDisabled: "grid.$$editor.form.readOnly",
			ngShow: "grid.$$editor.form.canEdit()",
			...
			class: "btn btn-primary btn-edit-popup"
		}, "<i class='fa fa-undo'></i>", container);
		...
		axTableEditor.createDeleteBtn(container);
		axTableEditor.createPrintBtn(container);
	...
	static createPrintBtn(toolbar) {
		createElement("ax-dropdown-popup", {
			...
			ngShow: "grid.$$editor.form.enabledExport()",
			ngDisabled: "!grid.$$editor.form.readOnly || !grid.$$editor.form.dataItem",
			btnClass: "btn btn-primary",
			btnHtml: "<i class=\"fa fa fa-upload\" ></i>",
			popupClass: "editor-export",
			caretClass: "fa ",
