(function () {
	...
	function controlTemplate($element, $attrs) {
		...
		$attrs.initialSelect = "'"+ trans($attrs.initialSelect, true) +"'";
		$element.attr("initial-select", $attrs.initialSelect );
		if (applicationInfo.theme.baseOn === "bootstrap4") {
			template = createElement("li", {
				class: "tab nav-item",
				"ng-repeat": "title in $tabsCtrl.tabsTitles",
				"ng-attr-tab-title": "{{::trans(title,true)}}",

				"ng-disabled": "$tabsCtrl.tabsDisabled({title:title})"
			}, createElement("a", {class:"nav-link", href:"#", "ng-bind": "::trans(title,true)"}), header);
		} else
			template = createElement("button", {
				class: "tab nav-item",
				"ng-repeat": "title in $tabsCtrl.tabsTitles",
				"ng-attr-tab-title": "{{::trans(title,true)}}",
				"ng-bind": "::trans(title,true)",
				"ng-disabled": "$tabsCtrl.tabsDisabled({title:title})"
			}, "", header);
		...
		tabsViews.each(function (index, view) {
			angular.element(view).addClass("tab-view").attr("tab-title", trans(view.getAttribute("tab-title"),true));
